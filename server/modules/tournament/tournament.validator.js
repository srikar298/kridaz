...
export const createTournamentSchema = z.object({
  body: z
    ...
    .refine(
      (data) => {
        if (data.advanceFee != null && data.entryFee != null) {
          return data.advanceFee <= data.entryFee;
        }
        return true;
      },
      {
        message: "Advance fee cannot exceed the full entry fee",
        path: ["advanceFee"],
      }
    )
    .refine(
...
export const updateTournamentSchema = z.object({
  body: z.object({
    ...
    advanceFee: z.coerce.number().min(0).optional(),
    prizePool: z.coerce.number().min(0).optional(),
    // CANCELLED added for tournament cancellation flow
