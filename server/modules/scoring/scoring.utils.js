...
export const computeScoreSnapshot = (scoring, match) => {
  ...
  const currentInnings = scoring.innings.find(
    (i) => i.inningsIndex === currentInningsIndex
  );
  ...
  const crr =
    totalValidBalls > 0
      ? (totalRuns / (totalValidBalls / ballsPerOver)).toFixed(2)
      : "0.00";
  ...
  const lastBalls = (scoring.timeline || []).slice(-6).map((ball) => {
    ...
    return {
      type: "run",
      label: suffix ? `${total}${suffix}` : `${total}`,
      isExtra: ball.isExtra,
    };
  });
  ...
  const getPlayerName = (userId) => {
    ...
    const slot = allSlots.find(
      (s) =>
        s.userId === userId || s.customPlayerId === userId || s.id === userId
    );
  ...
  // Active Batters
  const activeBatters = [scoring.strikerId, scoring.nonStrikerId]
    .filter((id) => id)
    .map((id) => {
      ...
      return {
        ...
        strikeRate:
          stat?.battingBalls > 0
            ? Number(((stat.battingRuns / stat.battingBalls) * 100).toFixed(2))
            : 0,
      };
    });
  ...
  if (scoring.bowlerId) {
    const bStat = scoring.playerStats.find(
      (s) => s.userId === scoring.bowlerId
    );
    currentBowler = {
      id: scoring.bowlerId,
      ...
      economy:
        bStat?.bowlingBalls > 0
          ? Number(
              ((bStat.bowlingRuns / bStat.bowlingBalls) * ballsPerOver).toFixed(
                2
              )
            )
          : 0,
  ...
  const battingTeamImage = isTeamABatting
    ? match.teamA?.image
    : match.teamB?.image;
  ...
  for (const ball of chronologicalTimeline) {
    ...
    if (validBallsInInnings > 0 && validBallsInInnings % ballsPerOver === 0) {
      ...
      if (!wormData.find((w) => w.over === currentOver)) {
        wormData.push({
          over: currentOver,
          runs: currentOversRuns,
          wickets: currentOversWickets,
        });
      }
    }
  }
  if (validBallsInInnings % ballsPerOver !== 0) {
    wormData.push({
      over: Number((validBallsInInnings / ballsPerOver).toFixed(1)),
      runs: currentOversRuns,
      wickets: currentOversWickets,
    });
  }
  ...
  const isAllOut =
    !houseRules.lastManStands && totalWickets >= wicketsForAllOut;
  const nrrEffectiveBalls =
    currentInnings.isCompleted && isAllOut
  ...
  const mvpLeaderboard = (scoring.playerStats || [])
    .map((stat) => {
      let points = 0;
      // Batting
      ...
      const sr =
        stat.battingBalls > 0
          ? (stat.battingRuns / stat.battingBalls) * 100
          : 0;
      if (stat.battingBalls >= w.srMinBalls && sr > w.srThreshold)
        points += w.srBonus;
      ...
      const econ =
        stat.bowlingBalls > 0
          ? (stat.bowlingRuns / stat.bowlingBalls) * ballsPerOver
          : 0;
      if (stat.bowlingBalls >= w.econMinBalls && econ < w.econThreshold)
        points += w.econBonus;
  ...
  const effectiveMaxOvers =
    currentInningsIndex === 1 && scoring.revisedOvers != null
      ? Number(scoring.revisedOvers)
      : maxOvers;
  const oversFinished =
    totalValidBalls >= Math.floor(effectiveMaxOvers * ballsPerOver);
  ...
  if (currentInningsIndex === 1) {
    ...
    if (isMatchComplete) {
      ...
      const defendingTeamName =
        chasingTeamName === teamAName ? teamBName : teamAName;
  ...
  return {
    ...
    lastBallRaw:
      scoring.timeline && scoring.timeline.length > 0
        ? scoring.timeline[0]
        : null,
    ...
    isLive:
      (match.scoringStatus || match.status) === "LIVE"
        ? true
        : (match.isLive ?? false),
    ...
    youtubeVideoId: match.youtubeLiveUrl
      ? extractYouTubeId(match.youtubeLiveUrl)
      : match.streamConfig?.youtubeVideoId
        ? extractYouTubeId(match.streamConfig.youtubeVideoId) ||
          match.streamConfig.youtubeVideoId
        : null,
    wormData,
    nrrEffectiveOvers,
    ...
    isSuperOver: currentInnings.isSuperOver ?? false,
    isFollowOn: currentInnings.isFollowOn ?? false,
    isInningsComplete,
    isMatchComplete,
    ...
    professionals: (() => {
      ...
      if (match.umpire?.name)
        ...
        pros.push({ name: match.customUmpire.name, role: "Umpire" });
      ...
      if (match.scorer?.name)
        ...
        pros.push({ name: match.customScorer.name, role: "Scorer" });
      ...
      if (match.streamer?.name)
        ...
        pros.push({ name: match.customStreamer.name, role: "Streamer" });
