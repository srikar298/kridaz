...
import { careerStatsQueue } from "../../queues/careerStats.queue.js";
...
import { computeScoreSnapshot, resolveHouseRules, DEFAULT_MVP_WEIGHTS } from "./scoring.utils.js";
...
const mapHostedGame = (hostedGame) => {
  ...
  const teamA = hostedGame.teams?.find((t) => t.teamKey === "teamA") || {
    name: "Team A",
    slots: [],
  };
  const teamB = hostedGame.teams?.find((t) => t.teamKey === "teamB") || {
    name: "Team B",
    slots: [],
  };

  const getCaptain = (team) => {
    const captainSlot = team.slots?.find(
      (s) =>
        s.role &&
        (s.role.trim().toLowerCase() === "captain" ||
          s.role.trim().toLowerCase() === "c" ||
          s.role.trim().toLowerCase() === "(c)")
    );
    if (captainSlot) {
      if (captainSlot.user)
        return {
          id: captainSlot.user.id,
          name: captainSlot.user.name,
          profilePicture: captainSlot.user.profilePicture,
          type: "user",
        };
      if (captainSlot.customPlayer)
        return {
          id: captainSlot.customPlayer.id,
          name: captainSlot.customPlayer.name,
          type: "custom",
        };
    }
    return null;
...
export const aggregatePlayerStats = async (matchScoring, hostedGame) => {
  try {
    ...
    matchScoring.timeline
      .filter((t) => t.fielderId)
      .forEach((t) => playerIds.add(t.fielderId));
    ...
    for (const stat of matchScoring.playerStats) {
      ...
      if (stat.bowlingBalls > 0) {
        ...
        cricket.runsConceded =
          (cricket.runsConceded || 0) + (stat.bowlingRuns || 0);
        cricket.ballsBowled =
          (cricket.ballsBowled || 0) + (stat.bowlingBalls || 0);

        const currentBest = cricket.bestBowling || { wickets: 0, runs: 999 };
        if (
          stat.bowlingWickets > currentBest.wickets ||
          (stat.bowlingWickets === currentBest.wickets &&
            stat.bowlingRuns < currentBest.runs)
        ) {
          cricket.bestBowling = {
            wickets: stat.bowlingWickets,
            runs: stat.bowlingRuns,
          };
        }

        cricket.bowlingEconomy =
          cricket.ballsBowled > 0
            ? Number(
                ((cricket.runsConceded / cricket.ballsBowled) * 6).toFixed(2)
              )
            : 0;
        cricket.bowlingAverage =
          cricket.wickets > 0
    ...
    // Offload Kridaz Career Stats & Badge Aggregation to background BullMQ worker
    try {
      await careerStatsQueue.add("AGGREGATE_MATCH_STATS", {
        matchScoring,
        hostedGame
      });
      logger.info(`[Scoring] Enqueued career stats aggregation for match: ${matchScoring.id}`);
    } catch (careerErr) {
      logger.error("Error enqueuing career stats in aggregatePlayerStats, falling back to sync:", careerErr);
      try {
        await CareerStatsService.aggregateMatchCareerStats(matchScoring, hostedGame);
      } catch (syncErr) {
        logger.error("Sync career stats aggregation failed:", syncErr);
      }
    }

    logger.info(
      `Successfully aggregated normalized stats for match ${matchScoring.id}`
    );
    return earnedBadges;
  } catch (error) {
...
export const goLiveSession = async (matchId, viewer) => {
  ...
  if (!process.env.OVERLAY_TOKEN_SECRET) {
    ...
    throw new InternalError(
      "Server configuration error: overlay token secret not set."
    );
  }
  ...
  const overlayConfig = hostedGame.overlayConfig || {
    showScoreboard: true,
    showCommentary: true,
  };
  try {
    ...
    logger.warn(
      "[Scoring] Redis state init failed (non-fatal):",
      redisErr.message
    );
  }
  ...
  try {
    ...
    logger.error(
      "Error caching/broadcasting live snapshot in goLiveSession:",
      err
    );
  }
...
export const endLiveSession = async (matchId, viewer) => {
  ...
  try {
    ...
    logger.warn(
      "[Scoring] Redis cleanup failed (non-fatal):",
      redisErr.message
    );
  }
  ...
  try {
    ...
    logger.error(
      "Error caching/broadcasting live snapshot in endLiveSession:",
      err
    );
  }
};
...
export const finalizeMatch = async (scoringId, viewer) => {
  ...
  const resultSnapshot = computeScoreSnapshot(
    scoring,
    mapHostedGame(hostedGameForResult)
  );
 ...
 */
export const initializeScoringSession = async (
  finalMatchId,
  finalBattingTeam,
  umpireId,
  userRole,
  ...
  if (!isAuthorized) {
    throw new ForbiddenError(
      "Authorization failed. Only the assigned umpire, host, or admin can score this match."
    );
  }
  ...
  if (timeDiffMs > thirtyDaysMs) {
    throw new BadRequestError(
      "Scoring can only be started within 30 days of the scheduled start time."
    );
  }
  if (timeDiffMs < -thirtyDaysMs) {
    throw new BadRequestError(
      "This match was scheduled over 30 days ago — please create a new one."
    );
  }
 ...
 */
export const advanceToNextInnings = async (scoringId, battingTeamId, options = {}, viewer) => {
  await verifyScoringAuth(scoringId, viewer);
  ...
  // Guard 1 — match already declared complete.
  if (scoring.status === "COMPLETED") {
    throw new BadRequestError("Cannot advance — match has been finalized.", {
      code: "MATCH_ALREADY_COMPLETE",
    });
  }

  const prevInnings = scoring.innings.find((i) => i.inningsIndex === scoring.currentInningsIndex);
  if (!prevInnings) {
    throw new BadRequestError(
      `Cannot advance — current innings ${scoring.currentInningsIndex} doesn't exist.`,
      { code: "NO_CURRENT_INNINGS" }
    );
  }

  // Guard 2 — same batting team guard. Allowed in Follow-on (Test cricket).
  if (battingTeamId === prevInnings.battingTeam && !isFollowOn) {
    throw new BadRequestError(
      "Next innings batting team must differ from current innings unless it is a Follow-on.",
      { code: "SAME_BATTING_TEAM" }
    );
  }

  const nextInningsIndex = scoring.currentInningsIndex + 1;

  await prisma.$transaction([
    prisma.innings.updateMany({
      where: { matchId: scoringId, inningsIndex: scoring.currentInningsIndex },
      data: { isCompleted: true },
    }),
    prisma.innings.create({
      data: {
        matchId: scoringId,
        inningsIndex: nextInningsIndex,
...
export const updateMatchStatus = async (scoringId, newStatus, viewer) => {
  ...
  if (!match)
    throw new NotFoundError("Match not found", { code: "MATCH_NOT_FOUND" });
  ...
  if (isPauseStatus && match.timerState === "RUNNING") {
    ...
    if (match.timerLastStartedAt) {
      const diffSecs = Math.floor(
        (now.getTime() - match.timerLastStartedAt.getTime()) / 1000
      );
    ...
    newStatus.toUpperCase() === "LIVE" &&
    (match.timerState === "PAUSED" || match.timerState === "NOT_STARTED")
  ) {
    timerUpdate = {
      timerState: "RUNNING",
  ...
  const liveData = computeScoreSnapshot(
    updatedScoring,
    mapHostedGame(hostedGame)
  );
  await liveStateService.setLiveScore(scoring.gameId, liveData);
...
export const useReview = async (
  ...
  if (!innings)
    throw new NotFoundError("Innings not found", { code: "INNINGS_NOT_FOUND" });

  const field =
    team === "batting" ? "battingTeamReviews" : "fieldingTeamReviews";
  const currentReviews = innings[field];

  if (currentReviews <= 0) {
    throw new ConflictError("No reviews remaining for this team", {
      code: "NO_REVIEWS_REMAINING",
    });
  }
...
export const updateActivePlayers = async (
  ...
  if (
    houseRules.enforceConsecutiveOverBlock &&
    bowlerId &&
    scoring.bowlerId === bowlerId
  ) {
  ...
  const finalStrikerId =
    strikerId !== undefined ? strikerId : scoring.strikerId;
  const finalNonStrikerId =
    nonStrikerId !== undefined ? nonStrikerId : scoring.nonStrikerId;
...
export const revertLastBall = async (scoringId, viewer) => {
  ...
  const currentInnings = scoring.innings.find(
    (i) => i.inningsIndex === lastBall.inningsIndex
  );
  if (!currentInnings) {
  ...
  if (lastBall.extraType === "WIDE")
    newExtras.wides = Math.max(0, newExtras.wides - extraRuns);
  else if (lastBall.extraType === "NO_BALL")
    newExtras.noBalls = Math.max(0, newExtras.noBalls - extraRuns);
  else if (lastBall.extraType === "BYE")
    newExtras.byes = Math.max(0, newExtras.byes - extraRuns);
  else if (lastBall.extraType === "LEG_BYE")
    newExtras.legByes = Math.max(0, newExtras.legByes - extraRuns);
  else if (lastBall.extraType === "PENALTY")
    newExtras.penalty = Math.max(0, newExtras.penalty - extraRuns);

  const ballsPerOver = houseRules.ballsPerOver || 6;

  let wasMaiden = false;
  if (isLegalBall && lastBall.ballInOver === ballsPerOver - 1) {
  ...
  await prisma.$transaction([
    ...
    prisma.matchPlayerStat.updateMany({
      where: {
        matchId: scoring.id,
        userId: lastBall.bowlerId,
        inningsIndex: lastBall.inningsIndex,
      },
      data: {
        bowlingRuns: {
          decrement: !["BYE", "LEG_BYE", "PENALTY"].includes(lastBall.extraType)
            ? runs + extraRuns
            : 0,
        },
    ...
    ...(lastBall.isWicket
      ? [
          prisma.matchPlayerStat.updateMany({
            where: {
              matchId: scoring.id,
              userId: lastBall.playerOutId || lastBall.batterId,
              inningsIndex: lastBall.inningsIndex,
            },
            data: {
              outStatus: "NOT_OUT",
...
export const processScoreUpdate = async (scoringId, ballData, viewer) => {
  ...
  const currentInnings = scoring.innings.find(
    (i) => i.inningsIndex === scoring.currentInningsIndex
  );
  if (!currentInnings) {
  ...
  if (isDeadBall) {
    ballData.runs = 0;
    ballData.extraRuns = 0;
    ballData.isExtra = false;
    ballData.extraType = "NONE";
    ballData.isWicket = false;
    ballData.wicketType = null;
    ballData.isBoundary = false;
    ballData.isFour = false;
    ballData.isSix = false;
  }
  ...
  // Law 25.6: on a wide, a batter can only be out via RUN_OUT, STUMPED,
  // OBSTRUCTING_FIELD, or HANDLED_BALL. Any other dismissal off a wide
  // is illegal and must be rejected server-side.
  const WIDE_LEGAL_WICKETS = new Set([
    'RUN_OUT', 'STUMPED', 'OBSTRUCTING_FIELD', 'HANDLED_BALL', 'TIMED_OUT'
  ]);
  if (isWide && ballData.isWicket && !WIDE_LEGAL_WICKETS.has(ballData.wicketType)) {
    throw new BadRequestError(
      `A batter cannot be out ${ballData.wicketType} off a wide ball (MCC Law 25.6).`,
      { code: 'ILLEGAL_DISMISSAL_WIDE' }
    );
  }
  ...
  if (ballData.isWicket) {
    const isRunOutOrRetired = [
      "RUN_OUT",
      "RETIRED_HURT",
      "RETIRED_OUT",
    ].includes(ballData.wicketType);

    if (isRunOutOrRetired) {
      let currentStrikerEndId = strikerId;
      let currentNonStrikerEndId = nonStrikerId;

      const crossed = ballData.didCross ?? (physicalRunsRan % 2 !== 0);
      if (crossed) {
  ...
  await prisma.$transaction([
    prisma.matchBall.create({
      data: {
        ...
        isOverthrow: ballData.isOverthrow || false,
        overthrowRuns: ballData.overthrowRuns || 0,
        isDeadBall: isDeadBall,
        obstructionMode: ballData.obstructionMode || null
      }
    }),
    prisma.innings.update({
      where: { id: currentInnings.id },
      data: {
        ...
        totalWickets:
          ballData.isWicket && ballData.wicketType !== "RETIRED_HURT"
            ? { increment: 1 }
            : undefined,
        extras: newExtras,
      },
    ...
    ...(bowlerId ? [
      prisma.matchPlayerStat.upsert({
        ...
        update: {
          ...
          bowlingWickets: { increment: (ballData.isWicket && !["RUN_OUT", "MANKAD", "RETIRED_HURT", "RETIRED_OUT", "TIMED_OUT", "OBSTRUCTING_FIELD", "HIT_BALL_TWICE", "HANDLED_BALL"].includes(ballData.wicketType)) ? 1 : 0 }
        },
        create: {
          matchId: scoring.id,
          userId: bowlerId,
          ...
          bowlingWickets: (ballData.isWicket && !["RUN_OUT", "MANKAD", "RETIRED_HURT", "RETIRED_OUT", "TIMED_OUT", "OBSTRUCTING_FIELD", "HIT_BALL_TWICE", "HANDLED_BALL"].includes(ballData.wicketType)) ? 1 : 0
        }
      })
    ] : []),
    prisma.cricketMatch.update({
  ...
  if (liveData.isInningsComplete && updatedScoring.timerState === "RUNNING") {
    ...
    const newElapsedTime =
      (updatedScoring.totalDurationSeconds || 0) + elapsedSinceLastStart;
  ...
  import("../commentary/commentary.service.js")
    ...
    .catch((err) =>
      logger.error("[Scoring] Failed to load commentary queue:", err)
    );
...
export const fetchMatchStatus = async (matchId) => {
  ...
  const scoring = await prisma.cricketMatch.findFirst({
    where: {
      OR: [{ id: resolvedId }, { gameId: resolvedId }],
    },
    include: {
      innings: true,
      playerStats: true,
  ...
  if (scoring) {
    ...
    customPlayers.forEach((c) =>
      userMap.set(c.id, { id: c.id, name: c.name, profilePicture: null })
    );
    ...
    const mappedHostedGame = mapHostedGame(hostedGame);
    const scoringSnapshot = computeScoreSnapshot(
      scoringWithUsers,
      mappedHostedGame
    );
...
export const fetchMatchAnalytics = async (matchId) => {
  ...
  const formattedTimeline = scoring.timeline.map((ball) => ({
    ...ball,
    batter: userMap.get(ball.batterId) || {
      name: "Batter",
      profilePicture: null,
    },
    bowler: userMap.get(ball.bowlerId) || {
      name: "Bowler",
      profilePicture: null,
    },
    fielder: ball.fielderId
      ? userMap.get(ball.fielderId) || { name: "Fielder", profilePicture: null }
      : null,
  }));
  ...
  // Use the same MVP weights as the live engine (DEFAULT_MVP_WEIGHTS from
  // scoring.utils.js) so the post-match MVP always agrees with the live ticker.
  const w = DEFAULT_MVP_WEIGHTS;
  const playerPoints = new Map();
  scoringWithUsers.playerStats.forEach(s => {
    const runs = s.battingRuns || s.runs || 0;
    const balls = s.battingBalls || 0;
    const wickets = s.bowlingWickets || s.wickets || 0;
    const bowlingBalls = s.bowlingBalls || 0;
    const bowlingRuns = s.bowlingRuns || 0;
    const strikeRate = balls > 0 ? (runs / balls) * 100 : 0;
    const economy = bowlingBalls > 0 ? (bowlingRuns / bowlingBalls) * 6 : 999;

    let pts = runs * w.perRun;
    if (runs >= 30) pts += w.bonus30;
    if (runs >= 50) pts += w.bonus50;
    if (runs >= 100) pts += w.bonus100;
    if (strikeRate >= w.srThreshold && balls >= w.srMinBalls) pts += w.srBonus;
    pts += wickets * w.perWicket;
    if (wickets >= 3) pts += w.bonus3w;
    if (wickets >= 5) pts += w.bonus5w;
    if (economy < w.econThreshold && bowlingBalls >= w.econMinBalls) pts += w.econBonus;
  ...
  scoringWithUsers.timeline.forEach((ball) => {
    if (ball.isWicket && ball.fielderId) {
      const current = playerPoints.get(ball.fielderId) || {
        name: ball.fielder?.name || "Fielder",
        points: 0,
        profilePicture: ball.fielder?.profilePicture,
      };
      current.points += 10;
  ...
  const sortedPlayers = Array.from(playerPoints.values()).sort(
  ...
  return {
    ...
    analytics: {
      ...
      totalFours: scoringWithUsers.playerStats.reduce(
        (acc, curr) => acc + (curr.battingFours || curr.fours || 0),
        0
      ),
      totalSixes: scoringWithUsers.playerStats.reduce(
        (acc, curr) => acc + (curr.battingSixes || curr.sixes || 0),
        0
      ),
    },
  };
...
export const fetchLiveScoreSnapshot = async (matchId) => {
  ...
  if (!snapshot) {
    ...
    if (match.umpire?.name)
      professionals.push({
        id: match.umpire.id,
        name: match.umpire.name,
        role: "Umpire",
        profilePicture: match.umpire.profilePicture,
      });
    else if (match.customUmpire?.name)
      professionals.push({ name: match.customUmpire.name, role: "Umpire" });

    if (match.scorer?.name)
      professionals.push({
        id: match.scorer.id,
        name: match.scorer.name,
        role: "Scorer",
        profilePicture: match.scorer.profilePicture,
      });
    else if (match.customScorer?.name)
      professionals.push({ name: match.customScorer.name, role: "Scorer" });

    if (match.streamer?.name)
      professionals.push({
        id: match.streamer.id,
        name: match.streamer.name,
        role: "Streamer",
        profilePicture: match.streamer.profilePicture,
      });
    else if (match.customStreamer?.name)
      professionals.push({ name: match.customStreamer.name, role: "Streamer" });

    if (Array.isArray(match.customProfessionals)) {
      match.customProfessionals.forEach((p) => {
        if (p?.name)
          professionals.push({ name: p.name, role: p.role || "Official" });
      });
    }
...
const generateUniqueShortId = async () => {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const existing = await prisma.hostedGame.findUnique({
...
export const createScoringMatch = async (userId, matchData) => {
  ...
  if (format === "TEST" || format === "5_DAY") {
    ...
    resolvedOversPerDay = customOversPerDay
      ? parseInt(customOversPerDay) || 20
      : 20;
  } else {
    resolvedDays = 1;
    resolvedOversPerDay =
  ...
  const game = await prisma.hostedGame.create({
    data: {
      ...
      time: matchDateTime
        ? new Date(matchDateTime).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          })
...
export const getUserScoringGames = async (userId) => {
  ...
  const games = await prisma.hostedGame.findMany({
    where: {
      OR: [{ hostId: userId }, { umpireId: userId }, { scorerId: userId }],
    },
    include: {
      teams: {
        include: {
...
export const getScoringGameById = async (gameId) => {
  ...
  const teamARecord = game.teams?.find(t => t.teamKey === 'teamA');
  const teamBRecord = game.teams?.find(t => t.teamKey === 'teamB');
  const required = game.maxMembers || 11;
  const countAssigned = (team) => {
    if (!team || !team.slots) return 0;
    return team.slots.filter(s => s.userId != null || s.customPlayerId != null).length;
  };
  const aAssigned = countAssigned(teamARecord);
  const bAssigned = countAssigned(teamBRecord);

  const readinessCheck = {
    teamA: {
      assignedPlayers: aAssigned,
      required,
      ready: aAssigned >= required
    },
    teamB: {
      assignedPlayers: bAssigned,
      required,
      ready: bAssigned >= required
    },
    hasToss: game.cricketMatch?.tossWinner != null,
    hasUmpire: game.umpireId != null
  };
...
export const toggleMatchTimer = async (scoringId, viewer) => {
  ...
  for (const stat of match.playerStats) {
    if (activeIds.includes(stat.userId)) {
      if (newTimerState === "RUNNING") {
        ...
        const timeDiff = stat.timerLastStartedAt
          ? Math.floor(
              (now.getTime() - stat.timerLastStartedAt.getTime()) / 1000
            )
          : 0;
        await prisma.matchPlayerStat.update({
          where: { id: stat.id },
  ...
  if (io) {
    ...
    const liveData = computeScoreSnapshot(
      fullUpdatedMatch,
      mapHostedGame(fullUpdatedMatch.game)
    );
...
export const addPenaltyRuns = async (scoringId, runs, teamId, viewer) => {
  ...
  const currentInnings = match.innings.find(
    (i) => i.inningsIndex === match.currentInningsIndex
  );
  if (!currentInnings)
    throw new ConflictError("Innings not active", {
      code: "INNINGS_NOT_ACTIVE",
    });
  ...
  if (!houseRules.penaltyEnabled) {
    throw new BadRequestError("Penalty runs are disabled for this match.", {
  ...
  // Recompute a proper snapshot and push it like every other scoring endpoint.
  // The old code used the wrong event name ('scoreUpdated' lowercase) and
  // emitted the raw match object — Flutter's LiveScoreSnapshot.fromJson
  // silently failed on that payload.
  const io = getIO();
  if (io) {
    const [fullScoring, hostedGame] = await Promise.all([
      prisma.cricketMatch.findUnique({
        where: { id: scoringId },
        include: {
          innings: true,
    ...
    await liveStateService.setLiveScore(match.gameId, liveData);
    io.to(match.gameId).emit(SOCKET.SCORE_UPDATED, liveData);
  }
...
export const updateHouseRules = async (scoringId, viewer, incoming) => {
  if (!viewer || (!viewer.id && viewer.role?.toLowerCase() !== "scorer")) {
    const err = new UnauthorizedError(
      "Authentication required to change house rules."
    );
    err.meta = { code: "AUTH_REQUIRED" };
    throw err;
  }
  ...
  const stored =
    scoring.houseRules && typeof scoring.houseRules === "object"
      ? scoring.houseRules
      : {};
  const merged = { ...stored };
  ...
  if (
    merged.ballsPerOver != null &&
    (!Number.isInteger(merged.ballsPerOver) ||
      merged.ballsPerOver < 1 ||
      merged.ballsPerOver > 12)
  ) {
    throw new BadRequestError(
      "ballsPerOver must be an integer between 1 and 12.",
      { code: "INVALID_BALLS_PER_OVER" }
    );
  }
  if (
    merged.playersPerTeam != null &&
    (!Number.isInteger(merged.playersPerTeam) ||
      merged.playersPerTeam < 2 ||
      merged.playersPerTeam > 30)
  ) {
    throw new BadRequestError(
      "playersPerTeam must be an integer between 2 and 30.",
      { code: "INVALID_PLAYERS_PER_TEAM" }
    );
  }
  if (
    merged.maxRunsPerBall != null &&
    (!Number.isInteger(merged.maxRunsPerBall) ||
      merged.maxRunsPerBall < 1 ||
      merged.maxRunsPerBall > 12)
  ) {
    throw new BadRequestError(
      "maxRunsPerBall must be an integer between 1 and 12.",
      { code: "INVALID_MAX_RUNS_PER_BALL" }
    );
  }
