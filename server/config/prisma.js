...
const processSocialAccounts = (accounts, action) => {
  ...
  return accounts.map((acc) => {
    ...
    if (updated.accessToken) {
      updated.accessToken =
        action === "encrypt"
          ? encrypt(updated.accessToken)
          : decrypt(updated.accessToken);
    }
    if (updated.refreshToken) {
      updated.refreshToken =
        action === "encrypt"
          ? encrypt(updated.refreshToken)
          : decrypt(updated.refreshToken);
    }
    return updated;
  });
};
  ...
  query: {
    ...
    user: {
      async $allOperations({ model, operation, args, query }) {
        ...
        const isWrite = ["create", "update", "upsert", "updateMany"].includes(
          operation
        );
        if (isWrite) {
          const processData = (data) => {
            ...
            if (data.socialAccounts) {
              data.socialAccounts = processSocialAccounts(
                data.socialAccounts,
                "encrypt"
              );
            }
