...
const Login = ({ isModal = false }) => {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn && !showOnboarding) {
      ...
      if (normalizedRole === "admin" || normalizedRole === "bmsp_admin") {
        dispatch(logout());
        toast.error(
          "Administrators must log in via the Platform Admin Console."
        );
      } else if (
  ...
  const content = (
    ...
    >
      {/* Logo */}
      <div className="absolute top-[38%] left-6 -translate-y-1/2 z-50">
        <img
          src="/logo1.png"
          alt="Logo"
          fetchPriority="high"
      ...
      <div className="flex-1 flex flex-col px-6 pt-4 pb-4 bg-transparent overflow-hidden z-10">
        ...
        >
          ...
          <div
            className={
              showOtpInput
                ? "flex flex-col gap-4 animate-fade-in flex-1"
                : "hidden"
            }
          >
            <div className="mb-2">
              <h3 className="text-[22px] font-bold text-white uppercase tracking-tight">
                Verification Code
              </h3>
            ...
            <div className="flex flex-col items-center mt-4 gap-3">
              {timeLeft > 0 ? (
                <p className="text-white/60 text-sm">
                  Resend in <span className="text-[#BFF367]">{timeLeft}s</span>
                </p>
              ) : (
                <p className="text-white/60 text-sm">
          ...
          {/* Email / Password Step */}
          <div
            className={!showOtpInput ? "flex flex-col gap-3 flex-1" : "hidden"}
          >
            {/* Email Input */}
            <div className="relative flex items-center">
              ...
              <div className="absolute left-4 text-[#C8F53B] drop-shadow-[0_0_8px_rgba(200,245,59,0.3)]">
                <User size={20} />
              </div>
              <input
                {...register("email")}
            ...
            <p className="text-center text-[14px] text-white/60 mt-5">
              ...
              <button
                type="button"
                onClick={() => (isModal ? toggleView() : navigate("/signup"))}
            ...
            {accountNotFound && (
              <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl animate-fade-in">
                ...
                <Link
                  to="/signup"
                  className="w-full bg-white/10 hover:bg-white/20 text-white h-11 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all"
                >
