...
import { ChevronLeft, X, ShieldCheck, Zap, User, ArrowRight } from "lucide-react";
...
const SignUp = ({ isModal = false }) => {
  ...
  const handleGoogleSuccess = async (googleResponse) => {
      ...
      try {
        ...
        const payload = { role: "user", inviteToken, umpireInvite, mode: "signup" };
  ...
  const content = (
      <div className={`relative z-10 w-full max-w-md mx-auto transition-all duration-1000 transform flex flex-col flex-1 h-full md:h-auto md:justify-center ${mounted ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"}`}>
                ...
                {step === 1 && (
                  <div className="flex-1 flex flex-col animate-fade-in">
                    ...
                    <div className="mt-auto pb-4 flex flex-col gap-5">
                      ...
                      >
                        {loading ? "Processing..." : <><span>Continue</span><ArrowRight size={18} /></>}
                      </button>
                    ...
                    <div className="flex-1">
                      <div className="flex flex-col items-start justify-center text-left mb-6">
                        <div className="space-y-1">
                          <p className="text-[12px] text-white/60">Enter OTP sent to {authMode === 'email' ? email : phone}</p>
                        </div>
                      </div>
    ...
    <div className={`fixed inset-0 z-[9999] bg-black transition-opacity duration-500 ${!mounted ? 'opacity-0' : 'opacity-100'}`}>
      <div className={`relative w-full h-full flex flex-col transition-all duration-500 ${!mounted ? 'translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden bg-black pointer-events-none">
          <img src="/signup-background.png" alt="Signup" className="w-full h-full object-contain object-top mt-10" />
