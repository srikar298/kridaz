        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-4 lg:gap-5">
          <div className="bg-[#121212] border border-white/10 rounded-[16px] p-2 md:p-3 flex flex-col relative overflow-hidden shadow-[var(--shadow-2)] hover:border-[#B3DC26]/30 transition-all duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-1 md:gap-0">
              ...
              <span className="px-1.5 md:px-2 py-0.5 bg-[#B3DC26]/10 text-[#B3DC26] text-[8px] md:text-[10px] font-medium uppercase tracking-wider rounded-full self-start md:self-auto">
                Active
              </span>
            </div>
          </div>
            ...
            <table className="w-full text-left table-fixed sm:table-auto">
              <thead>
                <tr className="border-b border-white/10 bg-[#151617]/50">
                  <th className="px-1 sm:px-3 lg:px-6 py-2 lg:py-5 text-[7px] sm:text-[10px] lg:text-[12px] font-medium text-[#999999] uppercase tracking-wider font-open-sans truncate w-[40%] sm:w-auto">
                    Customer
                  </th>
                  <th className="px-1 sm:px-3 lg:px-6 py-2 lg:py-5 text-[7px] sm:text-[10px] lg:text-[12px] font-medium text-[#999999] uppercase tracking-wider font-open-sans truncate w-[20%] sm:w-auto">
                    Bookings
                  </th>
                  <th className="px-1 sm:px-3 lg:px-6 py-2 lg:py-5 text-[7px] sm:text-[10px] lg:text-[12px] font-medium text-[#999999] uppercase tracking-wider font-open-sans truncate w-[20%] sm:w-auto">
                    Revenue
                  </th>
                  <th className="px-1 sm:px-3 lg:px-6 py-2 lg:py-5 text-[7px] sm:text-[10px] lg:text-[12px] font-medium text-[#999999] uppercase tracking-wider text-right font-open-sans truncate w-[20%] sm:w-auto">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#2D2D2D]/30">
                {filteredCustomers.length > 0 ? (
                  filteredCustomers.map((customer) => (
                    <tr
                      key={customer.id}
                      className="group hover:bg-[#1B1B1B]/20 transition-colors"
                    >
                      <td className="px-1 sm:px-3 lg:px-6 py-2 lg:py-4 truncate">
                        <div className="flex items-center gap-1.5 lg:gap-3">
                          <div
                            className={`w-5 h-5 lg:w-8 lg:h-8 rounded-[16px] flex items-center justify-center shrink-0 ${customer.isRegistered ? "bg-[#B3DC26]/20" : "bg-[#1B1B1B]"}`}
                          >
                            <User
                              className={`w-3 h-3 lg:w-4 lg:h-4 ${customer.isRegistered ? "text-[#B3DC26]" : "text-white/40"}`}
                            />
                          </div>
                          <div className="min-w-0">
                            <p className="text-[9px] sm:text-[12px] lg:text-[14px] font-semibold text-white tracking-tight group-hover:text-[#B3DC26] transition-colors font-inter truncate">
                              {customer.name}
                            </p>
                      ...
                      <td className="px-1 sm:px-3 lg:px-6 py-2 lg:py-4">
                        <span className="text-[8px] lg:text-[13px] font-medium text-[#999999] uppercase tracking-widest font-inter truncate block">
