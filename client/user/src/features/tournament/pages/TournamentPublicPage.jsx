...
import { Trophy, Calendar, MapPin, Users, IndianRupee, Share2, Info, FileText } from 'lucide-react';
  ...
  if (error || !tournament) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-white">
        ...
        <p className="text-sm text-white/50 mb-6">This tournament may have been canceled or the link is invalid.</p>
        <button onClick={() => navigate('/')} className="px-6 py-2 bg-[#BFF367] text-black rounded-full font-bold">Go Home</button>
      </div>
    );
  }

  const spotsLeft = Math.max(0, (tournament.maxTeams || 0) - (tournament.teams?.length || 0));
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 pt-8">
          ...
          <div className="flex flex-col items-center text-center mt-8">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 text-xs md:text-sm text-white/80 font-bold tracking-widest uppercase mb-8">
              <span className="flex items-center gap-1"><Trophy size={14} className="text-[#55DEE8]" /> {tournament.sport} • {tournament.type}</span>
              <span className="w-1 h-1 rounded-full bg-white/30" />
              <span className="flex items-center gap-1"><Users size={14} className="text-[#55DEE8]" /> {tournament.format}</span>
              {tournament.venues?.length > 0 && (
                <>
                  <span className="w-1 h-1 rounded-full bg-white/30" />
                  <span className="flex items-center gap-1"><MapPin size={14} className="text-[#55DEE8]" /> {tournament.venues[0].turf?.name || 'Multiple Venues'}</span>
                </>
              )}
            </div>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-[#111] border border-white/5 rounded-2xl p-4 md:p-6 mb-8 flex flex-wrap gap-6 md:gap-0 justify-between items-center relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 w-1/2 md:w-auto">
            ...
            <div>
              <p className="text-[10px] text-white/50 uppercase font-bold">Prize Pool</p>
              <p className="text-lg font-black text-white">â‚¹{tournament.prizePool?.toLocaleString() || 'TBA'}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 w-1/2 md:w-auto border-l border-white/5 pl-6 md:pl-8">
            ...
            <div>
              <p className="text-[10px] text-white/50 uppercase font-bold">Starts On</p>
              <p className="text-lg font-black text-white">
                {tournament.startDate ? new Date(tournament.startDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : 'TBA'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 w-full md:w-auto border-t md:border-t-0 md:border-l border-white/5 pt-6 md:pt-0 md:pl-8">
            ...
            <div>
              <p className="text-[10px] text-white/50 uppercase font-bold">Spots Left</p>
              <p className="text-lg font-black text-white">{spotsLeft} <span className="text-sm font-normal text-white/50">/ {tournament.maxTeams}</span></p>
            </div>
          </div>
        </div>
        ...
        <div className="flex overflow-x-auto hide-scrollbar border-b border-white/5 mb-6">
          {[
            ...
            { id: 'rules', label: 'Rules & Prizes', icon: <Trophy size={14} /> },
            { id: 'teams', label: 'Registered Teams', icon: <Users size={14} /> },
          ].map(tab => (
            <button
              key={tab.id}
            ...
            >
              ...
              {activeTab === tab.id && (
                <motion.div layoutId="pubTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#BFF367]" />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="min-h-[300px]">
          {activeTab === 'about' && (
            <div className="space-y-6 animate-fade-in text-sm text-white/70 leading-relaxed whitespace-pre-wrap">
              <h3 className="text-lg font-black text-white uppercase">About Tournament</h3>
              <p>{tournament.details?.about || 'No details provided.'}</p>
              
              {tournament.details?.facilities && (
                <div className="mt-6">
                  <h3 className="text-lg font-black text-white uppercase mb-2">Facilities Available</h3>
          ...
          {activeTab === 'rules' && (
            <div className="space-y-6 animate-fade-in text-sm text-white/70 leading-relaxed whitespace-pre-wrap">
              <h3 className="text-lg font-black text-white uppercase">Prizes</h3>
              <p>{tournament.details?.awards || 'Trophy and cash prize for winners.'}</p>
              
              <h3 className="text-lg font-black text-white uppercase mt-6">Rules</h3>
              <p>Standard {tournament.format} {tournament.sport} rules apply.</p>
            </div>
          )}

          {activeTab === 'teams' && (
            <div className="animate-fade-in">
              <h3 className="text-lg font-black text-white uppercase mb-4">Teams in the mix ({tournament.teams?.length || 0})</h3>
              
              {tournament.teams?.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {tournament.teams.map((t, idx) => (
                    <div key={idx} className="flex items-center gap-4 bg-[#111] p-4 rounded-xl border border-white/5">
                      <div className="w-12 h-12 rounded-full bg-[#1a1a1a] flex flex-shrink-0 items-center justify-center overflow-hidden border border-white/10">
                        {t.team?.logo ? (
                          <img src={t.team.logo} alt="team" className="w-full h-full object-cover" />
                        ) : (
                          <Users className="text-white/30" size={20} />
                        )}
                      </div>
                      <div>
                        <p className="font-bold text-white text-sm">{t.team?.name}</p>
                        <p className="text-[10px] text-white/50">{t.team?.city || 'Unknown City'}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center p-8 bg-[#111] rounded-2xl border border-white/5">
                  <p className="text-sm font-bold text-white/50 mb-2">No teams registered yet.</p>
                  <p className="text-xs text-[#BFF367]">Be the first to join!</p>
                </div>
              )}
            </div>
          )}
