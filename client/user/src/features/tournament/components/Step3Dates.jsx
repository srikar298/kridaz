...
const Step3Dates = ({ formData, onNext, onBack, isLoading }) => {
  const [localData, setLocalData] = useState({
    ...
    registrationStartDate: formData.registrationStartDate
      ? new Date(formData.registrationStartDate).toISOString().split("T")[0]
      : "",
    registrationEndDate: formData.registrationEndDate
      ? new Date(formData.registrationEndDate).toISOString().split("T")[0]
      : "",
    details: {
  ...
  const submit = () => {
    ...
    onNext({
      ...
      endDate: localData.endDate
        ...
        : null,
      registrationStartDate: localData.registrationStartDate
        ? new Date(localData.registrationStartDate).toISOString()
        : null,
      registrationEndDate: localData.registrationEndDate
        ? new Date(localData.registrationEndDate).toISOString()
        : null,
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {renderDateInput(
            ...
            localData.registrationStartDate
          )}
          {renderDateInput(
            "Registration End",
            "registrationEndDate",
            localData.registrationEndDate,
            true
          )}
        </div>
      </section>
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input
              ...
              className="w-full bg-card border border-white/5 rounded-xl px-4 py-4 text-sm font-bold text-white focus:outline-none focus:border-primary transition-colors"
            />
          </div>
          ...
          <label className="flex items-center gap-3 p-4 bg-card rounded-xl border border-white/5 cursor-pointer hover:border-white/20 transition-colors">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">
                Matches will only be scheduled on Saturday & Sunday
              </p>
            </div>
          </label>
        </div>
