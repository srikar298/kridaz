...
import { ArrowRight, ArrowLeft, Trophy, CheckCircle2, Wallet, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="text-center py-6">
        ...
        <h2 className="text-2xl font-black uppercase tracking-widest text-white">Review & Publish</h2>
        <p className="text-xs text-white/50 mt-2">Almost there! Review your tournament details before going live.</p>
      </div>

      <section className="bg-[#111] rounded-2xl border border-white/5 p-4 space-y-4">
        <h3 className="text-xs font-black text-[#55DEE8] uppercase tracking-widest border-b border-white/10 pb-2">Basic Info</h3>
        <div>
          <SummaryItem label="Name" value={formData.name} />
          <SummaryItem label="Sport" value={formData.sport} />
          <SummaryItem label="Format" value={`${formData.type || ''} - ${formData.format || ''}`} />
          <SummaryItem label="Teams" value={`Max ${formData.maxTeams || 0} Teams`} />
        </div>

        <h3 className="text-xs font-black text-[#BFF367] uppercase tracking-widest border-b border-white/10 pb-2 mt-6">Financials</h3>
        <div>
      ...
      <section className="space-y-4">
        <h3 className="text-sm font-black text-white/90 uppercase tracking-widest">Platform Fee</h3>
        
        <div className={`p-4 rounded-xl border transition-all ${useWallet ? 'bg-[#111] border-[#BFF367]' : 'bg-[#111] border-white/5'}`}>
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-white text-sm">Pay from KRIDAZ Wallet</p>
                <p className="text-[10px] text-white/50">Balance: â‚¹{walletBalance}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-sm font-black text-[#FFD700]">â‚¹{platformFee}</p>
