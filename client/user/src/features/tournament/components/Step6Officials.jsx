...
const Step6Officials = ({ formData, onNext, onBack, isLoading }) => {
  ...
  const renderOfficialCard = (role, icon) => {
    ...
    return (
      <div className={`p-4 rounded-xl border transition-all ${config.needed ? 'bg-[#111] border-[#BFF367]' : 'bg-[#0a0a0a] border-white/5 opacity-60'}`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${config.needed ? 'bg-[#BFF367] text-black' : 'bg-[#1a1a1a] text-white/50'}`}>
              {icon}
            </div>
        ...
        {config.needed && (
          <div className="flex items-center justify-between pt-3 border-t border-white/5">
            <span className="text-xs text-white/50">Number required per match</span>
            <div className="flex items-center gap-3">
              <button 
              ...
              <span className="text-sm font-black w-4 text-center">{config.count}</span>
              <button 
                onClick={() => handleCountChange(role, 1)}
        ...
        <label className="flex items-center gap-3 p-4 bg-[#111] rounded-xl border border-white/5 cursor-pointer hover:border-white/20 transition-colors mb-6">
          <div className="relative flex items-center justify-center">
            <input
              ...
              onChange={(e) => setLocalData(prev => ({ ...prev, details: { ...prev.details, requireKridazOfficials: e.target.checked } }))}
      ...
      <section className="space-y-4 pt-4 border-t border-white/5">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => setLocalData(prev => ({ ...prev, details: { ...prev.details, officialsPayment: 'Organizer' } }))}
            ...
            <button
              onClick={() => setLocalData(prev => ({ ...prev, details: { ...prev.details, officialsPayment: 'Teams' } }))}
              className={`flex-1 p-4 rounded-xl border text-center transition-all ${
