...
const ScheduleTab = ({ tournament }) => {
  const { data: matchesRes, isLoading: isLoadingMatches } = useGetTournamentMatchesQuery(tournament.id);
  ...
  if (isLoadingMatches) {
    return <div className="text-center p-8 text-white/50">Loading matches...</div>;
  }

  const groupStageMatches = matches.filter(m => m.tournamentStage === 'GROUP');
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-white/70">
              Let KRIDAZ automatically generate your group stage bracket based on {tournament.details?.maxTeams || 0} teams and {tournament.format} format.
            </p>
          </div>
          <button 
      ...
      <div className="bg-[#111] border border-white/5 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-sm font-black text-white/90 uppercase tracking-widest">Scheduled Matches</h3>
          <button 
        ...
        <div className="space-y-4">
          {matches.length === 0 ? (
            <div className="text-center p-8 border border-dashed border-white/10 rounded-xl">
              <p className="text-sm text-white/40 font-bold mb-2">No matches scheduled yet.</p>
              <button onClick={() => setShowAutoModal(true)} className="text-xs text-[#BFF367] hover:underline font-bold">Auto Generate Now</button>
            </div>
          ) : (
            matches.map((match, i) => (
              <div key={match.id} className="border border-white/10 rounded-xl p-4 hover:border-white/20 transition-colors">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-2 text-xs text-white/50">
                    <span className="flex items-center gap-1">
                      <Calendar size={12} /> 
                      {match.scheduledAt ? format(new Date(match.scheduledAt), 'MMM dd, yyyy') : 'TBD'}
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock size={12} /> 
                      {match.scheduledAt ? format(new Date(match.scheduledAt), 'HH:mm') : 'TBD'}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 w-1/3">
                    <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center font-bold text-xs">
                      {match.teams?.[0]?.team?.name?.substring(0, 2).toUpperCase() || 'TBD'}
                    </div>
                    <span className="font-bold">{match.teams?.[0]?.team?.name || 'TBD'}</span>
                  </div>
                  
                  <div className="text-xs font-black text-white/30 uppercase">VS</div>

                  <div className="flex items-center justify-end gap-3 w-1/3 text-right">
                    <span className="font-bold">{match.teams?.[1]?.team?.name || 'TBD'}</span>
                    <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center font-bold text-xs">
                      {match.teams?.[1]?.team?.name?.substring(0, 2).toUpperCase() || 'TBD'}
                    </div>
                  </div>
                </div>
      ...
      {showAutoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#111] border border-white/10 rounded-3xl p-6 max-w-md w-full relative">
            ...
            <h3 className="text-lg font-black uppercase tracking-widest mb-4">Schedule Preferences</h3>
            
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-white/50 uppercase tracking-widest mb-2 block">Start Date</label>
                <input 
                  type="date" 
                  value={startDate}
              ...
              <div>
                <label className="text-xs font-bold text-white/50 uppercase tracking-widest mb-2 block">Match Time Slots (Per Day)</label>
