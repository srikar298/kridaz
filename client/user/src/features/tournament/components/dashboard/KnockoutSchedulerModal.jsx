...
const KnockoutSchedulerModal = ({ tournament, onClose }) => {
  const { data: standingsRes, isLoading: isLoadingStandings } =
    useGetStandingsQuery(tournament.id);
  const standings = standingsRes?.data || [];

  const [manualSchedule, { isLoading: isScheduling }] =
    useManualScheduleMutation();
  ...
  const handleSaveMatch = async () => {
    if (!team1 || !team2) {
      return toast.error(
        "Please drag and drop two teams to schedule the match."
      );
    }
        ...
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-[#1a1a1a]">
          <div>
            ...
            <p className="text-xs text-white/50">
              Drag teams from the standings into the match slots to pair them
              manually.
            </p>
          </div>
          <button
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  >
                    {team1 ? (
                      ...
                      <span className="text-xs font-bold text-white/30 uppercase tracking-widest text-center">
                        Drag Team A Here
                      </span>
                    )}
                  </div>

                  <div className="flex-shrink-0 text-xl font-black text-white/20 italic px-2">
                    VS
                  </div>
