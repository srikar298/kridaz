...
import { Users, Search, ShieldAlert, MessageCircle, CheckCircle2, XCircle } from 'lucide-react';
...
const TeamsTab = ({ tournament }) => {
  ...
  // Mock teams data
  const teams = [
    { id: 1, name: 'Deccan Chargers', captain: 'Rohit S.', status: 'Approved', paid: true, players: 15 },
    { id: 2, name: 'Hyderabad Heroes', captain: 'Venkat R.', status: 'Pending', paid: false, players: 12 },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-[#111] border border-white/5 rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-white/5">
          {teams.map(team => (
            <div key={team.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 items-center hover:bg-white/5 transition-colors">
              <div className="col-span-4 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#BFF367]/20 to-[#55DEE8]/20 flex items-center justify-center border border-white/10">
                  <span className="font-black text-white/70">{team.name.charAt(0)}</span>
                </div>
                <div>
                  <p className="font-bold text-sm text-white">{team.name}</p>
                  <p className="text-[10px] text-white/50 flex items-center gap-1 mt-0.5">
                    {team.paid ? (
                      <span className="text-[#BFF367] flex items-center gap-1"><CheckCircle2 size={10} /> Paid Entry Fee</span>
                    ) : (
                      <span className="text-red-400 flex items-center gap-1"><XCircle size={10} /> Unpaid</span>
                    )}
                  </p>
                </div>
              </div>
