...
const TurfCard = ({ turf, featured = false, distance = "1.2km Away" }) => {
  ...
  const activeSlots = Array.isArray(parsedSlots)
    ...
    : [];
  const slotsLeft =
    turf.slotsLeft !== undefined ? turf.slotsLeft : activeSlots.length;
  ...
  const toggleWishlist = async (e) => {
    ...
    try {
      ...
      await axiosInstance.post("/api/user/turf/user/like", {
        turfId: targetId,
      });
    } catch (err) {
          ...
          <div className="space-y-3 md:space-y-4">
            ...
            <div className="flex items-center justify-between items-end pt-2 md:pt-3 border-t border-white/10">
              <div className="flex flex-col">
                <span className="text-[7px] md:text-[9px] font-black text-white/40 uppercase tracking-[0.2em] mb-0.5 md:mb-1 font-inter">
                  Starting
                </span>
