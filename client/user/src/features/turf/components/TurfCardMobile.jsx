...
const TurfCardMobile = ({ turf, distance: distanceProp }) => {
  ...
  const isWishlisted =
    isLoggedIn && savedData?.turfs?.some((t) => (t.id || t._id) === targetId);

  const returnTo = searchParams.get("returnTo");
  const baseTo = `/venue/${targetId}`;
  const to = returnTo
    ? `${baseTo}?returnTo=${encodeURIComponent(returnTo)}`
    : baseTo;
    ...
    const fetchBookedTimes = async () => {
      ...
      try {
        ...
        if (response.data && response.data.bookedTime) {
          const formattedBookedTime = response.data.bookedTime.map(
            (booking) => ({
              ...booking,
  ...
  useEffect(() => {
    ...
    const venueLat =
      turf.location?.coordinates?.[1] ?? turf.latitude ?? turf.lat;
    const venueLng =
      turf.location?.coordinates?.[0] ?? turf.longitude ?? turf.lng;
  ...
  const getSelectedSlotPrice = () => {
    ...
    if (slotIndex !== -1) {
      return (
        activeSlots[slotIndex]?.price ?? turf.pricePerHour ?? turf.price ?? null
      );
    }
            ...
            onClick={(e) => {
              ...
              if (navigator.share) {
                ...
                navigator.clipboard.writeText(
                  window.location.origin + "/venue/" + targetId
                );
            ...
            <span>
              {selectedSlot
                ? currentPrice !== null &&
                  ...
                  currentPrice !== ""
                  ? `${returnTo ? "Add to Game" : "Book Now"} - ₹${currentPrice}`
                  : returnTo
                    ? "Add to Game"
                    : "Book Now"
                : "Select a Time Slot"}
            ...
            >
              ...
              <div className="flex flex-col mb-6 shrink-0 bg-card p-[16px] rounded-[16px] border border-[rgba(255,255,255,0.08)]">
                ...
                <div className="grid grid-cols-7 gap-1 text-center mb-2">
                  {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
                    <div
                      key={d}
                      className="text-[12px] text-[rgba(255,255,255,0.70)] font-[400] py-1 uppercase tracking-wider"
                    >
                      {d}
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-7 gap-1 text-center">
                  {calendarDays.map((day, idx) => {
                    ...
                    const isInitiallySelected =
                      !selectedDate.dateObj &&
                      day.getDate() === selectedDate.dateNum &&
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          ...
                          setSelectedDate({
                            ...
                            dateNum: day.getDate(),
                            month: day.toLocaleDateString("en-US", {
                              month: "short",
                            }),
                          });
                        }}
              ...
              <div className="grid grid-cols-2 gap-[12px] mb-6 shrink-0">
                {displaySlots.length > 0 ? (
                  ...
                  <div className="col-span-2 text-center text-[12px] text-[rgba(255,255,255,0.70)] py-4 italic">
