        ...
        <>
          <div className="flex overflow-x-auto gap-4 pb-6 snap-x scroll-smooth no-scrollbar hover:no-scrollbar">
            {reviews.map((review, idx) => (
              ...
              >
                <div className="absolute -inset-0.5 bg-gradient-to-r from-primary/20 to-lime-600/0 rounded-[8px] blur opacity-0 group-hover:opacity-100 transition duration-500"></div>
                <div
                  className="relative h-full bg-zinc-900/40 border border-zinc-800 group-hover:border-primary/50 rounded-[8px] p-6 transition-all duration-300 animate-fade-in flex flex-col justify-between gap-6"
                ...
                >
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`w-3.5 h-3.5 ${i < review.rating ? "text-primary fill-primary" : "text-zinc-800"}`}
                          />
                        ))}
                      </div>
                    ...
                    <div className="relative">
                      <Quote className="absolute -top-2 -left-2 w-8 h-8 text-primary/5 opacity-20" />
                  ...
                  <div className="flex items-center gap-4 pt-4 border-t border-zinc-800/50">
                    ...
                    <div>
                      <Link
                        to={`/profile/${review.user?.id || review.user?._id}`}
                        className="font-black uppercase text-[10px] tracking-widest text-white mb-0.5 hover:text-primary transition-colors relative z-10"
                      >
          ...
          <div className="flex justify-center gap-2 mt-2 md:hidden">
            {reviews.slice(0, Math.min(reviews.length, 5)).map((_, i) => (
              <div
                key={i}
                className={`h-1 rounded-full transition-all duration-300 ${i === 0 ? "w-4 bg-primary" : "w-1 bg-zinc-800"}`}
              ></div>
            ))}
          </div>
        </>
